...
def main():
    ...
    with NVTXUtil("E2E", "red", mm), SynchronizeUtil(torchutil.torch_stream):
        ...
        with NVTXUtil("inference_wo_output", "red", mm), SynchronizeUtil(torchutil.torch_stream):
            for i, data in enumerate(dataset):
                with NVTXUtil(f"loop{i}", "blue", mm), SynchronizeUtil(torchutil.torch_stream):
                    if i >= opt.num_test:  # only apply our model to opt.num_test images.
                        break
        ...
        with NVTXUtil("inference_w_output", "red", mm), SynchronizeUtil(torchutil.torch_stream):
            for i, data in enumerate(dataset):
                with NVTXUtil(f"loop{i}", "blue", mm), SynchronizeUtil(torchutil.torch_stream):
                    if i >= opt.num_test:  # only apply our model to opt.num_test images.
                        break
